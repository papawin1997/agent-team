...
และ `docs/check/2026-09-25-multi-job-resume-v3-tb-scrutinize.html` (อ้างเป็น v3#1–v3#5)
...
- field ใหม่ใน `State` ทุกตัวไม่บังคับมี จึงยังเป็น `version: 1` ได้
  ...
  - `lastBuildAt?: string` (ISO 8601): `FileStateStore.save` ตั้ง `lastBuildAt = now` เฉพาะตอน `state.phase === 'BUILD'`
    **และ** `Object.values(state.progress).some(p => p.rounds > 0)` (#5, v2#3)
    ...
    รวมถึงการเข้า BUILD **รอบที่สอง** หลังขอแก้ตอน DELIVER (v3#2): `initProgress` (`domain.ts:59-60`) เก็บ `rounds` ของ task
    ที่ done และไม่ได้ changed ไว้ ดังนั้นการ save ตอนกด confirm ใน REVIEW จะตั้ง `lastBuildAt` ทันทีก่อนมีการแก้โค้ด
    "เพิ่งเข้า BUILD ยังไม่ตั้ง" จึงจริงเฉพาะการเข้า BUILD ครั้งแรก ยอมรับได้เพราะงานนั้นกำลังจะแก้โค้ดอยู่แล้ว
...
- `selectJob` ลบงานเปล่าทิ้งให้อัตโนมัติก่อนแสดงเมนู โดย **เรียก `lock(id)` ก่อน ถ้าได้ lock ค่อย `remove(id)`**
  ถ้า lock ไม่ได้ (อีก process กำลังใช้) ให้ข้ามไป ไม่ตัดสินจาก `JobInfo.lock` ที่อ่านมาก่อนหน้า (v3#1)
...
5. ถ้า `rename` ได้ `ENOENT` (อีก process ที่เริ่มพร้อมกันย้ายไฟล์นั้นไปแล้ว) ให้ข้ามไฟล์นั้นแล้วทำต่อ ไม่ถือว่าเป็น error (v3#5)
...
1. `repo.migrateLegacy()` → `repo.list()` → ลบงานเปล่า (lock ก่อนแล้วค่อยลบ ตามหัวข้อ "งานเปล่า") → คัดเฉพาะงานค้าง (phase ไม่ใช่ `DONE`/`ABORTED` และไม่ใช่งานเปล่า)
...
3. **ใส่ `opts.resume`:** เลือกงานค้างที่ใหม่สุดซึ่งไม่ได้ถูก lock แล้ว `lock()` และคืนงานนั้นโดยไม่แสดงเมนู (ดูหัวข้อ Lock)
   ทุกครั้งให้ `io.say('ทำต่องาน "<ชื่อ>" (<id>)')` เพื่อให้ user รู้ว่าได้งานไหน โดยเฉพาะเมื่องานใหม่สุดถูก lock อยู่
   แล้วระบบข้ามไปเลือกงานที่เก่ากว่า (v3#3)
...
- `d<n>` → ถ้างานแสดงว่าถูก lock ให้บอกว่ากำลังรันอยู่แล้วแสดงเมนูใหม่ ถ้าไม่ได้ถูก lock ให้ถามยืนยันด้วย **`io.ask` ตรง ๆ** (#6)
  `ลบงาน "<ชื่อ>" ถาวรใช่ไหม? (y/n)` รับ `y`/`yes`/`n`/`no` (ตอบอย่างอื่นให้ถามซ้ำ)
  ถ้าตอบ yes ให้ **เรียก `lock(id)` ก่อน** (v3#1: สถานะ lock ในเมนูอาจเก่าแล้ว เพราะ user อาจใช้เวลาตอบเป็นนาที
  ระหว่างนั้นอีก process อาจเลือกงานนี้ไปแล้ว) ถ้า lock ไม่ได้ ให้บอกว่ากำลังรันอยู่ (พร้อม path ของ lock) แล้วแสดงเมนูใหม่
  ถ้าได้ lock ค่อย `remove(id)` (lock ถูกลบไปพร้อมโฟลเดอร์) แล้วกลับไปขั้น 1 (ถ้าลบจนไม่เหลืองานค้างจะเข้าขั้น 2 คือเริ่มงานใหม่)
  ถ้าตอบ no ให้แสดงเมนูใหม่
...
- แยก handler ออกเป็นฟังก์ชันที่เทสต์ได้ใน `src/interrupt.ts` (v3#4):
  ```ts
  makeInterruptHandler(deps: {
    ...
    exit: (code: number) => void;   // index.ts ส่ง process.exit เทสต์ส่งตัวปลอม
  }): (signal: 'SIGINT' | 'SIGHUP') => void
  ```
  `index.ts` ลงทะเบียน handler ที่ได้กับทั้ง `SIGINT` และ `SIGHUP` (v2#1)
- handler อ่าน `jobId` จาก `getJobId()` ซึ่งอาจยังไม่มี (optional): ถ้ามีให้ `unlockSync(jobId)` แล้วพิมพ์
...
- ย้ายไฟล์ legacy ไม่สำเร็จ → throw พร้อมบอก path รอบถัดไปจะย้ายต่อจากเดิมได้ (#4) ยกเว้น `ENOENT` ซึ่งแปลว่าอีก process ย้ายไปแล้ว ให้ข้าม (v3#5)
...
- `tests/jobs.test.ts` (ใช้ temp dir จริง): create/list/remove, `updatedAt` ถูกตั้งทุกครั้งที่ save,
  ...
  artifact ค้างที่ root โดยไม่มี `state.json` → WARN ไม่ throw, rename ได้ ENOENT → ข้ามแล้วทำต่อ (v3#5),
...
- `tests/interrupt.test.ts`: เรียก `makeInterruptHandler(...)` ด้วย repo จริงใน temp dir และ `exit` ปลอม (v3#4)
  ทั้ง SIGINT/SIGHUP แล้ว **ไม่มี `run.lock` เหลือ** (v2#1), `exit` ถูกเรียกด้วย 130, ถ้ายังไม่มี jobId ก็ไม่ throw
- `tests/job-menu.test.ts` (ใช้ `ScriptedIO` + repo ใน temp dir): ไม่มีงานค้างก็เริ่มใหม่เลย / `--resume` เลือกงานล่าสุด /
  ...
  งานเปล่าไม่ขึ้นในเมนูและถูกลบ / งานเปล่าที่อีก process lock อยู่ไม่ถูกลบ /
  **งานถูก lock หลังจากแสดงเมนูแล้ว เมื่อกด `d` + `y` → ถูกปฏิเสธและโฟลเดอร์ยังอยู่** (v3#1) /
  `--resume` พิมพ์ชื่องานที่เลือก (v3#3) / คำเตือนขึ้นเฉพาะเมื่องานอื่นมี `lastBuildAt` ใหม่กว่า (การคุยกับ PM อย่างเดียวไม่ทำให้ขึ้นคำเตือน) /
